// This file was auto-generated by Fern from our API Definition.

import type { BaseClientOptions, BaseRequestOptions } from "../../../../../../../../BaseClient.js";
import { mergeHeaders, mergeOnlyDefinedHeaders } from "../../../../../../../../core/headers.js";
import * as core from "../../../../../../../../core/index.js";
import * as environments from "../../../../../../../../environments.js";
import * as errors from "../../../../../../../../errors/index.js";
import * as serializers from "../../../../../../../../serialization/index.js";
import * as BasisTheory from "../../../../../../../index.js";

export declare namespace Verify {
    export interface Options extends BaseClientOptions {}

    export interface RequestOptions extends BaseRequestOptions {}
}

export class Verify {
    protected readonly _options: Verify.Options;

    constructor(_options: Verify.Options) {
        this._options = _options;
    }

    /**
     * Initiates the cardholder verification flow for a pending enrollment.
     *
     * @param {string} enrollmentId
     * @param {BasisTheory.StartVerificationRequest} request
     * @param {Verify.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link BasisTheory.BadRequestError}
     * @throws {@link BasisTheory.UnauthorizedError}
     * @throws {@link BasisTheory.ForbiddenError}
     * @throws {@link BasisTheory.NotFoundError}
     * @throws {@link BasisTheory.ConflictError}
     * @throws {@link BasisTheory.UnprocessableEntityError}
     * @throws {@link BasisTheory.InternalServerError}
     *
     * @example
     *     await client.agentic.enrollments.verify.start("enrollment_id", {
     *         deviceContext: {
     *             screenHeight: 1,
     *             screenWidth: 1,
     *             userAgentString: "user_agent_string",
     *             languageCode: "language_code",
     *             timeZone: "time_zone",
     *             javaScriptEnabled: true,
     *             clientDeviceId: "client_device_id",
     *             clientReferenceId: "client_reference_id",
     *             platformType: "WEB"
     *         }
     *     })
     */
    public start(
        enrollmentId: string,
        request: BasisTheory.StartVerificationRequest,
        requestOptions?: Verify.RequestOptions,
    ): core.HttpResponsePromise<BasisTheory.VerificationResponse> {
        return core.HttpResponsePromise.fromPromise(this.__start(enrollmentId, request, requestOptions));
    }

    private async __start(
        enrollmentId: string,
        request: BasisTheory.StartVerificationRequest,
        requestOptions?: Verify.RequestOptions,
    ): Promise<core.WithRawResponse<BasisTheory.VerificationResponse>> {
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({
                "BT-TRACE-ID": requestOptions?.correlationId ?? this._options?.correlationId,
                "BT-API-KEY": requestOptions?.btApiKey ?? this._options?.btApiKey,
                ...(await this._getCustomAuthorizationHeaders()),
            }),
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.BasisTheoryEnvironment.Default,
                `enrollments/${core.url.encodePathParam(enrollmentId)}/verify`,
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryParameters: requestOptions?.queryParams,
            requestType: "json",
            body: serializers.StartVerificationRequest.jsonOrThrow(request, {
                unrecognizedObjectKeys: "strip",
                omitUndefined: true,
            }),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return {
                data: serializers.VerificationResponse.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    skipValidation: true,
                    breadcrumbsPrefix: ["response"],
                }),
                rawResponse: _response.rawResponse,
            };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new BasisTheory.BadRequestError(_response.error.body, _response.rawResponse);
                case 401:
                    throw new BasisTheory.UnauthorizedError(_response.error.body, _response.rawResponse);
                case 403:
                    throw new BasisTheory.ForbiddenError(_response.error.body, _response.rawResponse);
                case 404:
                    throw new BasisTheory.NotFoundError(_response.error.body, _response.rawResponse);
                case 409:
                    throw new BasisTheory.ConflictError(_response.error.body, _response.rawResponse);
                case 422:
                    throw new BasisTheory.UnprocessableEntityError(_response.error.body, _response.rawResponse);
                case 500:
                    throw new BasisTheory.InternalServerError(_response.error.body, _response.rawResponse);
                default:
                    throw new errors.BasisTheoryError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        switch (_response.error.reason) {
            case "non-json":
                throw new errors.BasisTheoryError({
                    statusCode: _response.error.statusCode,
                    body: _response.error.rawBody,
                    rawResponse: _response.rawResponse,
                });
            case "timeout":
                throw new errors.BasisTheoryTimeoutError(
                    "Timeout exceeded when calling POST /enrollments/{enrollment_id}/verify.",
                );
            case "unknown":
                throw new errors.BasisTheoryError({
                    message: _response.error.errorMessage,
                    rawResponse: _response.rawResponse,
                });
        }
    }

    /**
     * Choose the OTP delivery method (SMS, email, etc.) for enrollment verification.
     *
     * @param {string} enrollmentId
     * @param {BasisTheory.agentic.enrollments.SelectMethodRequest} request
     * @param {Verify.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link BasisTheory.BadRequestError}
     * @throws {@link BasisTheory.UnauthorizedError}
     * @throws {@link BasisTheory.ForbiddenError}
     * @throws {@link BasisTheory.NotFoundError}
     * @throws {@link BasisTheory.UnprocessableEntityError}
     * @throws {@link BasisTheory.InternalServerError}
     *
     * @example
     *     await client.agentic.enrollments.verify.method("enrollment_id", {
     *         methodId: "method_id"
     *     })
     */
    public method(
        enrollmentId: string,
        request: BasisTheory.agentic.enrollments.SelectMethodRequest,
        requestOptions?: Verify.RequestOptions,
    ): core.HttpResponsePromise<BasisTheory.VerificationResponse> {
        return core.HttpResponsePromise.fromPromise(this.__method(enrollmentId, request, requestOptions));
    }

    private async __method(
        enrollmentId: string,
        request: BasisTheory.agentic.enrollments.SelectMethodRequest,
        requestOptions?: Verify.RequestOptions,
    ): Promise<core.WithRawResponse<BasisTheory.VerificationResponse>> {
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({
                "BT-TRACE-ID": requestOptions?.correlationId ?? this._options?.correlationId,
                "BT-API-KEY": requestOptions?.btApiKey ?? this._options?.btApiKey,
                ...(await this._getCustomAuthorizationHeaders()),
            }),
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.BasisTheoryEnvironment.Default,
                `enrollments/${core.url.encodePathParam(enrollmentId)}/verify/method`,
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryParameters: requestOptions?.queryParams,
            requestType: "json",
            body: serializers.agentic.enrollments.SelectMethodRequest.jsonOrThrow(request, {
                unrecognizedObjectKeys: "strip",
                omitUndefined: true,
            }),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return {
                data: serializers.VerificationResponse.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    skipValidation: true,
                    breadcrumbsPrefix: ["response"],
                }),
                rawResponse: _response.rawResponse,
            };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new BasisTheory.BadRequestError(_response.error.body, _response.rawResponse);
                case 401:
                    throw new BasisTheory.UnauthorizedError(_response.error.body, _response.rawResponse);
                case 403:
                    throw new BasisTheory.ForbiddenError(_response.error.body, _response.rawResponse);
                case 404:
                    throw new BasisTheory.NotFoundError(_response.error.body, _response.rawResponse);
                case 422:
                    throw new BasisTheory.UnprocessableEntityError(_response.error.body, _response.rawResponse);
                case 500:
                    throw new BasisTheory.InternalServerError(_response.error.body, _response.rawResponse);
                default:
                    throw new errors.BasisTheoryError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        switch (_response.error.reason) {
            case "non-json":
                throw new errors.BasisTheoryError({
                    statusCode: _response.error.statusCode,
                    body: _response.error.rawBody,
                    rawResponse: _response.rawResponse,
                });
            case "timeout":
                throw new errors.BasisTheoryTimeoutError(
                    "Timeout exceeded when calling POST /enrollments/{enrollment_id}/verify/method.",
                );
            case "unknown":
                throw new errors.BasisTheoryError({
                    message: _response.error.errorMessage,
                    rawResponse: _response.rawResponse,
                });
        }
    }

    /**
     * Submit the one-time password received by the cardholder.
     *
     * @param {string} enrollmentId
     * @param {BasisTheory.agentic.enrollments.SubmitOtpRequest} request
     * @param {Verify.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link BasisTheory.BadRequestError}
     * @throws {@link BasisTheory.UnauthorizedError}
     * @throws {@link BasisTheory.ForbiddenError}
     * @throws {@link BasisTheory.NotFoundError}
     * @throws {@link BasisTheory.UnprocessableEntityError}
     * @throws {@link BasisTheory.InternalServerError}
     *
     * @example
     *     await client.agentic.enrollments.verify.otp("enrollment_id", {
     *         otpCode: "otp_code"
     *     })
     */
    public otp(
        enrollmentId: string,
        request: BasisTheory.agentic.enrollments.SubmitOtpRequest,
        requestOptions?: Verify.RequestOptions,
    ): core.HttpResponsePromise<BasisTheory.Enrollment> {
        return core.HttpResponsePromise.fromPromise(this.__otp(enrollmentId, request, requestOptions));
    }

    private async __otp(
        enrollmentId: string,
        request: BasisTheory.agentic.enrollments.SubmitOtpRequest,
        requestOptions?: Verify.RequestOptions,
    ): Promise<core.WithRawResponse<BasisTheory.Enrollment>> {
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({
                "BT-TRACE-ID": requestOptions?.correlationId ?? this._options?.correlationId,
                "BT-API-KEY": requestOptions?.btApiKey ?? this._options?.btApiKey,
                ...(await this._getCustomAuthorizationHeaders()),
            }),
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.BasisTheoryEnvironment.Default,
                `enrollments/${core.url.encodePathParam(enrollmentId)}/verify/otp`,
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryParameters: requestOptions?.queryParams,
            requestType: "json",
            body: serializers.agentic.enrollments.SubmitOtpRequest.jsonOrThrow(request, {
                unrecognizedObjectKeys: "strip",
                omitUndefined: true,
            }),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return {
                data: serializers.Enrollment.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    skipValidation: true,
                    breadcrumbsPrefix: ["response"],
                }),
                rawResponse: _response.rawResponse,
            };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new BasisTheory.BadRequestError(_response.error.body, _response.rawResponse);
                case 401:
                    throw new BasisTheory.UnauthorizedError(_response.error.body, _response.rawResponse);
                case 403:
                    throw new BasisTheory.ForbiddenError(_response.error.body, _response.rawResponse);
                case 404:
                    throw new BasisTheory.NotFoundError(_response.error.body, _response.rawResponse);
                case 422:
                    throw new BasisTheory.UnprocessableEntityError(_response.error.body, _response.rawResponse);
                case 500:
                    throw new BasisTheory.InternalServerError(_response.error.body, _response.rawResponse);
                default:
                    throw new errors.BasisTheoryError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        switch (_response.error.reason) {
            case "non-json":
                throw new errors.BasisTheoryError({
                    statusCode: _response.error.statusCode,
                    body: _response.error.rawBody,
                    rawResponse: _response.rawResponse,
                });
            case "timeout":
                throw new errors.BasisTheoryTimeoutError(
                    "Timeout exceeded when calling POST /enrollments/{enrollment_id}/verify/otp.",
                );
            case "unknown":
                throw new errors.BasisTheoryError({
                    message: _response.error.errorMessage,
                    rawResponse: _response.rawResponse,
                });
        }
    }

    /**
     * Complete the verification flow (e.g. after passkey creation). Body is optional — Visa sends empty body, Mastercard sends assurance_data.
     *
     * @param {string} enrollmentId
     * @param {BasisTheory.agentic.enrollments.CompleteVerificationRequest} request
     * @param {Verify.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link BasisTheory.BadRequestError}
     * @throws {@link BasisTheory.UnauthorizedError}
     * @throws {@link BasisTheory.ForbiddenError}
     * @throws {@link BasisTheory.NotFoundError}
     * @throws {@link BasisTheory.UnprocessableEntityError}
     * @throws {@link BasisTheory.InternalServerError}
     *
     * @example
     *     await client.agentic.enrollments.verify.complete("enrollment_id")
     */
    public complete(
        enrollmentId: string,
        request: BasisTheory.agentic.enrollments.CompleteVerificationRequest = {},
        requestOptions?: Verify.RequestOptions,
    ): core.HttpResponsePromise<BasisTheory.Enrollment> {
        return core.HttpResponsePromise.fromPromise(this.__complete(enrollmentId, request, requestOptions));
    }

    private async __complete(
        enrollmentId: string,
        request: BasisTheory.agentic.enrollments.CompleteVerificationRequest = {},
        requestOptions?: Verify.RequestOptions,
    ): Promise<core.WithRawResponse<BasisTheory.Enrollment>> {
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            this._options?.headers,
            mergeOnlyDefinedHeaders({
                "BT-TRACE-ID": requestOptions?.correlationId ?? this._options?.correlationId,
                "BT-API-KEY": requestOptions?.btApiKey ?? this._options?.btApiKey,
                ...(await this._getCustomAuthorizationHeaders()),
            }),
            requestOptions?.headers,
        );
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.BasisTheoryEnvironment.Default,
                `enrollments/${core.url.encodePathParam(enrollmentId)}/verify/complete`,
            ),
            method: "POST",
            headers: _headers,
            contentType: "application/json",
            queryParameters: requestOptions?.queryParams,
            requestType: "json",
            body: serializers.agentic.enrollments.CompleteVerificationRequest.jsonOrThrow(request, {
                unrecognizedObjectKeys: "strip",
                omitUndefined: true,
            }),
            timeoutMs: (requestOptions?.timeoutInSeconds ?? this._options?.timeoutInSeconds ?? 60) * 1000,
            maxRetries: requestOptions?.maxRetries ?? this._options?.maxRetries,
            abortSignal: requestOptions?.abortSignal,
        });
        if (_response.ok) {
            return {
                data: serializers.Enrollment.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    skipValidation: true,
                    breadcrumbsPrefix: ["response"],
                }),
                rawResponse: _response.rawResponse,
            };
        }

        if (_response.error.reason === "status-code") {
            switch (_response.error.statusCode) {
                case 400:
                    throw new BasisTheory.BadRequestError(_response.error.body, _response.rawResponse);
                case 401:
                    throw new BasisTheory.UnauthorizedError(_response.error.body, _response.rawResponse);
                case 403:
                    throw new BasisTheory.ForbiddenError(_response.error.body, _response.rawResponse);
                case 404:
                    throw new BasisTheory.NotFoundError(_response.error.body, _response.rawResponse);
                case 422:
                    throw new BasisTheory.UnprocessableEntityError(_response.error.body, _response.rawResponse);
                case 500:
                    throw new BasisTheory.InternalServerError(_response.error.body, _response.rawResponse);
                default:
                    throw new errors.BasisTheoryError({
                        statusCode: _response.error.statusCode,
                        body: _response.error.body,
                        rawResponse: _response.rawResponse,
                    });
            }
        }

        switch (_response.error.reason) {
            case "non-json":
                throw new errors.BasisTheoryError({
                    statusCode: _response.error.statusCode,
                    body: _response.error.rawBody,
                    rawResponse: _response.rawResponse,
                });
            case "timeout":
                throw new errors.BasisTheoryTimeoutError(
                    "Timeout exceeded when calling POST /enrollments/{enrollment_id}/verify/complete.",
                );
            case "unknown":
                throw new errors.BasisTheoryError({
                    message: _response.error.errorMessage,
                    rawResponse: _response.rawResponse,
                });
        }
    }

    protected async _getCustomAuthorizationHeaders(): Promise<Record<string, string | undefined>> {
        const apiKeyValue = (await core.Supplier.get(this._options.apiKey)) ?? process?.env["BT-API-KEY"];
        return { "BT-API-KEY": apiKeyValue };
    }
}
