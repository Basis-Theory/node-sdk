// This file was auto-generated by Fern from our API Definition.

import type * as BasisTheory from "../../api/index.js";
import * as core from "../../core/index.js";
import type * as serializers from "../index.js";

export const CredentialsCard: core.serialization.ObjectSchema<
    serializers.CredentialsCard.Raw,
    BasisTheory.CredentialsCard
> = core.serialization.object({
    number: core.serialization.string().optional(),
    expirationMonth: core.serialization.property("expiration_month", core.serialization.number().optional()),
    expirationYear: core.serialization.property("expiration_year", core.serialization.number().optional()),
    cvc: core.serialization.string().optional(),
});

export declare namespace CredentialsCard {
    export interface Raw {
        number?: string | null;
        expiration_month?: number | null;
        expiration_year?: number | null;
        cvc?: string | null;
    }
}
